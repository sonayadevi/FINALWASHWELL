...
import {DataCuciExpress} from "./DataCuci";
import mesincuciimage from "../../assets/mesincuci.png";

const CuciExpressScreen = ({ navigation }) => {  //flatlist

  const [quantities, setQuantities] = useState(new Array(DataCuciExpress.length).fill(0));
  ...
  const totalPrice = quantities.reduce((total, quantity, index) => total + quantity * DataCuciExpress[index].price, 0);

  const handleOrder = () => {
    const orderItems = quantities.map((quantity, index) => ({
      ...DataCuciExpress[index],
      quantity,
      jenisCuci: 'Cuci Express',
    }));
  ...
  return (
    <>
      <Header title={"Cuci Express"} withBack="True" />
      <FlatList
        data={DataCuciExpress}
...
export default CuciExpressScreen;
