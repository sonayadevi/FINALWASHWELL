...
import {DataCuciVIP} from "./DataCuci";
import mesincuciimage from "../../assets/mesincuci.png";

const CuciVIPScreen = ({ navigation }) => {  //flatlist

  const [quantities, setQuantities] = useState(new Array(DataCuciVIP.length).fill(0));
  ...
  const totalPrice = quantities.reduce((total, quantity, index) => total + quantity * DataCuciVIP[index].price, 0);
  ...
  return (
    <>
      <Header title={"Cuci VIP"} withBack="True" />
      <FlatList
        data={DataCuciVIP}
